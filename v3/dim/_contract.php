<?php
	...
	$host = 'https://online.moysklad.ru/api/remap/1.1/entity/contract';
	$headers = array(
    ...
    do {
		...
		foreach($result[rows] as $key => $value){
			...
			$stat_result[$resultCount]['sum'] = $value[sum];
			$stat_result[$resultCount]['contractType'] = $value[contractType];
			...
			$stat_result[$resultCount]['agent_href'] = $value[agent][meta][href];
			$stat_result[$resultCount]['ownAgent_href'] = $value[ownAgent][meta][href];
			$stat_result[$resultCount]['organizationAccount_href'] = $value[organizationAccount][meta][href];
			$stat_result[$resultCount]['rate_href'] = $value[rate][meta][href];
	...
	header('Content-Disposition: attachment; filename=Договоры.csv');
