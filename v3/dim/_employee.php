<?php
	...
	$host = 'https://online.moysklad.ru/api/remap/1.1/entity/employee';
	$headers = array(
    ...
    do {
		...
		foreach($result[rows] as $key => $value){
			...
			$stat_result[$resultCount]['uid'] = $value[uid];
			$stat_result[$resultCount]['email'] = $value[email];
			$stat_result[$resultCount]['phone'] = $value[phone];
			$stat_result[$resultCount]['fullName'] = $value[fullName];
	...
	header('Content-Disposition: attachment; filename=Сотрудники.csv');
