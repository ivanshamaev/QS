<?php
	...
	$host = 'https://online.moysklad.ru/api/remap/1.1/entity/service';
	$headers = array(
    ...
    do {
		...
		foreach($result[rows] as $key => $value){
			...
			$stat_result[$resultCount]['pathName'] = $value[pathName];
			$stat_result[$resultCount]['minPrice'] = $value[minPrice];
			//href
			$stat_result[$resultCount]['owner_href'] = $value[owner][meta][href];
			$stat_result[$resultCount]['group_href'] = $value[group][meta][href];
			$stat_result[$resultCount]['productFolder_href'] = $value[productFolder][meta][href];
			$stat_result[$resultCount]['uom_href'] = $value[uom][meta][href];
	...
	header('Content-Disposition: attachment; filename=Услуги.csv');
