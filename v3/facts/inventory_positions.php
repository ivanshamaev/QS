<?php
	...
	if( $mode == "data" ) {
		...
		foreach($loss_positions as $position_key => $position_value){
				...
				foreach($result[rows] as $key_result => $value_result){
					...
					$stat_result[$resultCount]['inventory_id'] = $loss_id; 
					...
					$stat_result[$resultCount]['correctionAmount'] = $value_result[correctionAmount];
					$stat_result[$resultCount]['calculatedQuantity'] = $value_result[calculatedQuantity];
					$stat_result[$resultCount]['correctionSum'] = $value_result[correctionSum];
	...
	$printNameCSV = "ИнвентаризацияДетали";
